...
output_dir = "../../FILE/ALGORITMOS/ARVORE_DECISAO"
...
modelo_path = f"{output_dir}/modelo_arvore_decisao.pkl"
...
with open(f"{output_dir}/resultados_teste_arvore_decisao.txt", "w") as f:
...
plt.savefig(f"{grafico_dir}/matriz_confusao_teste_arvore_decisao.png")
plt.close()
...
df_test.to_csv(f"{output_dir}/previsoes_teste_arvore_decisao.csv", sep=";", index=False)
df[["Filme", "Previsao_Oscar"]].to_csv(f"{output_dir}/previsoes_completas_arvore_decisao.csv", sep=";", index=False)
